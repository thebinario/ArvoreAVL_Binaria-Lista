...
void inserirNoListaSimp(NoListaSimp **ATUAL, int x)
{
	...
	//Inserir elemento nas demais posiÁıes da lista
	else
	{
...
//Buscar posiÁ„o na lista L para inserir o valor x quando L nÔøΩo estÔøΩ vazia e x nÔøΩo ÔøΩ o menor valor de L.
...
void InsereValorListaSimpEncadeada(ListaSimpEncadeada *L, int x)
{
	...
	{
		//Inserir um valor x que ÔøΩ o menor valor da lista.
		contador_insercao_lista++;
		if (x < (*L).inicio->valor)
		{
		...
		//Inserir um valor x nas demais posiÁıes da lista.
		contador_insercao_lista++;
		if (x > (*L).inicio->valor)
		{
...
//////FUN«√O BUSCA  ELEMENTOS NA LISTA
...
{ //AVL
	...
	else
	{
		...
		{
			//	printf("Elemento j√° existente na arvore\n");
		}
